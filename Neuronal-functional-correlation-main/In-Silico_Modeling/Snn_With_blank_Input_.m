...
opt.TAll=1590; % Total simulation time, in sec 
...
for sstt=1:SimTimes
    ...
    for i = 1:nN
        ...
        J(i, neighbors) = g * randn(1, length(neighbors)) / sqrt(local_range * 2);
    end
    ...
    [Fr,S,frac]=runSNN_v2_showfrac(ini,J,nT,tau,E,inputWN,dT);
...
for i = 1:length(start_edge)
    stimuili_label_ind(i) = mod(i-1, 6) + 1;
end
stimuli_array = stiSqc;
stimuli_array_with_label = stiSqc_with_label;

save_file_name = fullfile(outpath, 'visual_stimuli_with_label24_withblank.mat');
